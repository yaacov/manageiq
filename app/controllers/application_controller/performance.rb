module ApplicationController::Performance
  ...
  def perf_set_or_fix_dates(options)
    ...
    if !options[:hourly_date].present? || (options[:hourly_date].present? && # Need to clear hourly date if not nil so it will be reset below if
    ...
    if !options[:daily_date].present? || (options[:daily_date].present? &&
